...
#define LIST_INSERT_BEFORE(listelm, elm, field)                                                    \
	do {                                                                                       \
		(elm)->field.le_prev = (listelm)->field.le_prev;         /*set arrow (the original elm before listelm)<-elm*/                \
		LIST_NEXT((elm), field) = (listelm);                     /*set arrow elm->listelm*/                          \
		*(listelm)->field.le_prev = (elm);                       /*set arrow (the original elm before listelm)->elm*/                         \
		(listelm)->field.le_prev = &LIST_NEXT((elm), field);     /*set arrow elm<-listelm*/                          \
	} while (0)
